...
export class LoginService{
    ...
    async GetAllUser(): Promise<{userId: number, userName: string, group:string, email: string}[]> {
        return await fetch(`${this.apiPath}/all`, {method: "GET", headers: {Authorization: `Bearer ${getToken()}`}})
            .then(res => res.json())
            .then(res => {return res as {userId: number, userName: string, group:string, email: string}[]});
    }
